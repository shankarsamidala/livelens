import { app, BrowserWindow, Tray, Menu, nativeImage, ipcMain, shell, systemPreferences, screen, desktopCapturer, session } from "electron"
...
type STTProvider = (GoogleSTT | RestSTT | DeepgramStreamingSTT | SonioxStreamingSTT | ElevenLabsStreamingSTT | OpenAIStreamingSTT | LiveLensProSTT) & {
  finalize?: () => void;
...
export class AppState {
  ...
  private createSTTProvider(speaker: 'interviewer' | 'user'): STTProvider | null {
    ...
    if (sttProvider === 'natively') {
      ...
      if (!nativelyKey) {
        // LiveLens is Coming Soon — no key means degrade gracefully like every other provider
        console.warn(`[Main] No LiveLens API Key configured for ${speaker}, falling back to GoogleSTT`);
        ...
        stt = new LiveLensProSTT(nativelyKey, speaker === 'interviewer' ? 'system' : 'mic');
      }
      ...
      // OpenAI STT: prefer dedicated STT key, fall back to the shared LLM key
    ...
    stt.on('error', (err: Error) => {
      ...
      const isAuthError = httpStatus === 401
        ...
        || msg.includes('could not load')       // Google ADC: "Could not load the default credentials"
        || msg.includes('default credentials')  // Google ADC fallback phrase
        || msg.includes('unauthorized');        // generic 401 text
    ...
    // Auto language detection: LiveLensProSTT emits 'languageDetected' when the
    ...
    if (stt instanceof LiveLensProSTT) {
  ...
  public async startMeeting(metadata?: any): Promise<void> {
    ...
    if (process.platform === 'darwin') {
      ...
      if (screenStatus === 'denied') {
        ...
        const message = 'Screen Recording permission denied. System audio will not be captured. To fix: System Settings → Privacy & Security → Screen Recording → enable LiveLens.';
  ...
  public setRecognitionLanguage(key: string): void {
    ...
    // 'auto' is only meaningful for LiveLensProSTT — other providers fall back to en-US.
    const sttProvider = CredentialsManager.getInstance().getSttProvider();
  ...
  private async withScreenshotCaptureSession<T>(
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, process.platform === 'darwin' ? 200 : 40));
  ...
  public setUndetectable(state: boolean): void {
    ...
    if (process.platform === 'darwin') {
      ...
      this._dockDebounceTimer = setTimeout(() => {
        ...
        if (settled) {
          // Capture whether LiveLens is currently the frontmost app BEFORE
          ...
          // If LiveLens was the focused window when the user toggled stealth,
...
async function initializeApp() {
  ...
  // Inject Content-Security-Policy on every renderer response.
  // All external network calls are made from the main process; the renderer only
  // needs to reach localhost (Vite dev server) and load local file:// resources.
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    callback({
      responseHeaders: {
        ...details.responseHeaders,
        'Content-Security-Policy': "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' http://localhost:* ws://localhost:*; media-src 'self' blob:; worker-src 'self' blob:; object-src 'none'; base-uri 'self'",
      },
    });
  });
  ...
  //   1. The LiveLens launcher window is visible and focused when the TCC dialog
          ...
          BrowserWindow.getAllWindows().forEach((win: Electron.BrowserWindow) => {
            if (!win.isDestroyed()) {
              win.webContents.send(
                ...
                'Screen Recording is disabled. System audio capture will not work. Click "Open Settings" to enable it, then restart LiveLens.'
              );
            }
          });
        } else {
