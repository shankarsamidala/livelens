...
export class WindowHelper {
  ...
  public createWindow(): void {
    ...
    const overlaySettings: Electron.BrowserWindowConstructorOptions = {
      ...
      // NSPanel with nonactivating behaviour: clicks inside the overlay don't
      // transfer the active-app role away from VSCode/browser/etc. When the
      // overlay hides, macOS returns focus to whatever was active before — not
      // the Desktop. Without this, hide() causes macOS to pick a new key window
      // and sometimes switches Spaces, making screenshots capture empty desktop.
    ...
    if (process.platform === "darwin") {
      ...
      // Deferred to ready-to-show: getNativeWindowHandle() returns immediately
      // after new BrowserWindow, but [NSView window] may be nil before Electron
      // attaches the view. Calling now races and the Rust side returns
      // "NSView has no associated NSWindow". ready-to-show guarantees the window
      // is live. Optional — silently skipped if native module is unavailable.
      ...
      // 'floating' level is not enough to render above fullscreen browser windows (F11).
      // 'screen-saver' uses a higher TOPMOST priority that wins against window-mode fullscreen apps.
  ...
  private setupWindowListeners(): void {
    ...
    if (this.overlayWindow) {
      ...
      // Re-assert always-on-top on blur (Windows only). Screen-sharing tools (Zoom, Teams)
      // can demote even HWND_TOPMOST windows. Re-applying 'screen-saver' on blur keeps the
      // overlay above the share surface. Skipped on macOS — triggers [NSApp activate].
  ...
  public hideMainWindow(): void {
    // Set opacity to 0 immediately — compositor stops rendering the window so
    // desktopCapturer sees through it cleanly.
    this.launcherWindow?.setOpacity(0);
    this.overlayWindow?.setOpacity(0);
    if (process.platform === 'darwin') {
      // On macOS do NOT call hide(). hide() deactivates the app and macOS picks
      // the "next" app — but the overlay is visibleOnAllWorkspaces with no fixed
      // Space, so macOS falls back to Desktop/Finder. The cropper then appears
      // over empty desktop instead of VSCode.
      //
      // Instead keep the window "shown" but fully passthrough: the cropper
      // (screen-saver level) sits above it and VSCode stays visible behind it.
      // switchToOverlay/switchToLauncher undo this with setIgnoreMouseEvents(false).
      this.overlayWindow?.setIgnoreMouseEvents(true, { forward: true });
      this.launcherWindow?.setIgnoreMouseEvents(true, { forward: true });
    } else {
      this.launcherWindow?.hide()
  ...
  public switchToOverlay(inactive?: boolean): void {
    ...
    if (this.overlayWindow && !this.overlayWindow.isDestroyed()) {
      ...
      if (process.platform === 'win32' && this.contentProtection) {
        ...
        this.overlayWindow.setOpacity(1);
        // Undo the passthrough set by hideMainWindow on macOS
        if (process.platform === 'darwin') {
          this.overlayWindow.setIgnoreMouseEvents(false);
        }
        this.overlayWindow.setContentProtection(this.contentProtection);
  ...
  public switchToLauncher(inactive?: boolean): void {
    ...
    if (this.launcherWindow && !this.launcherWindow.isDestroyed()) {
      if (process.platform === 'win32' && this.contentProtection) {
        ...
        // Undo the passthrough set by hideMainWindow on macOS
        if (process.platform === 'darwin') {
          this.launcherWindow.setIgnoreMouseEvents(false);
        }
        this.launcherWindow.setContentProtection(this.contentProtection);
        if (this.shouldFocusOverlay(inactive)) {
  ...
  private createCompactWindow(): void {
    ...
    if (process.platform === 'darwin') {
      ...
      this.compactWindow.setAlwaysOnTop(true, 'screen-saver');
    }
