...
export class CropperWindowHelper {
    ...
    private applyOpacityShield(): void {
        ...
        if (process.platform === 'win32') {
            ...
            this.cropperWindow.setContentProtection(this.isUndetectable);
            // showInactive keeps the cropper visible above everything (screen-saver level)
            // without re-activating live-lens. Combined with app.hide() in hideMainWindow,
            // this prevents macOS from switching to live-lens's home Space (Desktop).
            // ESC is handled via before-input-event on webContents so no focus needed.
