...
const CompactMode: React.FC = () => {
    ...
    return (
        <div className="w-full h-full bg-transparent flex items-start justify-center pt-[4px]">
            <div
                ref={pillRef}
                className="w-full mx-[4px] rounded-[14px] overflow-hidden"
                style={{
                    background: 'rgba(13,15,20,0.94)',
                    border: '1px solid rgba(255,255,255,0.10)',
                    boxShadow: '0 8px 40px rgba(0,0,0,0.70), 0 2px 8px rgba(0,0,0,0.40), 0 0 0 0.5px rgba(255,255,255,0.04)',
            ...
            >
                ...
                <div
                    ...
                    onClick={() => !isOpen && open()}
                >
                    <Search size={13} className="shrink-0" style={{ color: 'rgba(226,229,237,0.28)' }} />
                    <input
                        ref={inputRef}
                        type="text"
                        value={query}
                        ...
                        className="flex-1 bg-transparent border-none outline-none text-[13px] no-drag"
                        style={{ color: 'rgba(226,229,237,0.85)', caretColor: '#d97757' }}
                        autoComplete="off"
                        spellCheck={false}
                    />
                    {/* ⌘K hint */}
                    {!isOpen && (
                        <div className="flex items-center gap-[3px] shrink-0 no-drag">
                            {['⌘', 'K'].map(k => (
                                <span key={k} className="text-[10px] px-[5px] py-[1px] rounded-[4px]"
                                    style={{ color: 'rgba(226,229,237,0.35)', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
                                    {k}
                                </span>
                            ))}
                        </div>
                    ...
                    <button
                        ...
                        className="w-[24px] h-[24px] flex items-center justify-center rounded-[6px] border border-transparent no-drag transition-all duration-100"
                        style={{ color: 'rgba(226,229,237,0.28)' }}
                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.07)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.09)'; e.currentTarget.style.color = 'rgba(226,229,237,0.70)'; }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'transparent'; e.currentTarget.style.color = 'rgba(226,229,237,0.28)'; }}
                        ...
                        >
                            <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }} className="py-2">
                                ...
                                <div className="px-2 pb-1">
                                    <div className="px-2 pb-1 text-[10px] font-[700] tracking-[0.08em] uppercase"
                                        style={{ color: 'rgba(226,229,237,0.22)' }}>Explore</div>

                                    {/* AI Query */}
                                    <button
                                        className="w-full flex items-center gap-3 px-2 py-[7px] rounded-[8px] text-left"
                                ...
                                {sessionResults.length > 0 && (
                                    <div className="px-2 pt-1" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                                        <div className="px-2 pb-1 text-[10px] font-[700] tracking-[0.08em] uppercase"
                                            style={{ color: 'rgba(226,229,237,0.22)' }}>Sessions</div>
                                        <AnimatePresence initial={false} mode="popLayout">
                                            {sessionResults.map((r, i) => (
                                                <motion.button
                                                    ...
                                                    className="w-full flex items-center gap-3 px-2 py-[7px] rounded-[8px] text-left"
                                                    style={{ background: selectedIndex === i + 2 ? 'rgba(255,255,255,0.07)' : 'transparent' }}
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="text-[12.5px] truncate" style={{ color: 'rgba(226,229,237,0.80)' }}>{r.title}</div>
                                                        <div className="text-[11px]" style={{ color: 'rgba(226,229,237,0.30)' }}>{r.subtitle}</div>
                                                    </div>
                                                </motion.button>
                                            ))}
                                        </AnimatePresence>
