...
import { useActionButtonMode } from '../hooks/useActionButtonMode';
...
import { useSttNotConfigured } from '../hooks/useSttNotConfigured';
...
const LiveLensInterface: React.FC<LiveLensInterfaceProps> = ({
    onEndMeeting,
    overlayOpacity = getDefaultOverlayOpacity(),
}) => {
    const isLightTheme = false; // overlay is always dark glass regardless of system theme
    const [isExpanded, setIsExpanded] = useState(true);
    const [inputValue, setInputValue] = useState('');
    const { shortcuts, isShortcutPressed } = useShortcuts();
    const [messages, setMessages] = useState<Message[]>([]);
    const { status: sttInterviewerStatus, error: sttInterviewerError, provider: sttInterviewerProvider } = useSttStatus();
    ...
    useEffect(() => {
        isInterviewerCapturingRef.current = isInterviewerCapturing;
    }, [isInterviewerCapturing]);
    const [conversationContext, setConversationContext] = useState<string>('');
    // Analytics State
    const requestStartTimeRef = useRef<number | null>(null);

    const rollingTranscriptRef = useRef(''); // accumulates live transcript text from STT finals
    ...
    const [autoScroll, setAutoScroll] = useState(() => localStorage.getItem('liveLens_auto_scroll') !== 'false');
    ...
    const { actionButtonMode } = useActionButtonMode();
    ...
    useEffect(() => {
        const v = overlayOpacity ?? getDefaultOverlayOpacity();
        setLocalOpacity(v);
    ...
    const appearance = useMemo(
        () => getOverlayAppearance(localOpacity, isLightTheme ? 'light' : 'dark'),
        [localOpacity, isLightTheme]
    );
    ...
    const handleSolve = async () => {
        ...
        try {
            ...
            setMessages((prev) => {
                ...
                if (last?.isStreaming)
                    return [...prev.slice(0, -1), { id: String(Date.now()), role: 'system', text: `❌ Error: ${err}` }];
                return prev;
            });
        }
    };
    ...
    const { isMousePassthrough, setIsMousePassthrough } = useMousePassthrough();
    ...
    const sttNotConfiguredFromIpc = useSttNotConfigured();
    const [sttDismissed, setSttDismissed] = useState(false);
    // Reset dismiss whenever the IPC value changes so re-entering unconfigured state re-shows the banner
    useEffect(() => { setSttDismissed(false); }, [sttNotConfiguredFromIpc]);
    const sttNotConfigured = sttNotConfiguredFromIpc && !sttDismissed;

    // Auto-resize Window (ResizeObserver + attachedContext + 600 ms safety)
    useOverlayResize(contentRef, attachedContext);
        ...
        try {
            ...
            setMessages((prev) => {
                ...
                if (last && last.isStreaming && last.text === '') {
                    return prev
                        .slice(0, -1)
                        .concat({ id: Date.now().toString(), role: 'system', text: `❌ Error: ${err}` });
                }
                ...
                setMessages((prev) => {
                    ...
                    if (lastMsg && lastMsg.isStreaming && lastMsg.role === 'system') {
                        ...
                        updated[prev.length - 1] = {
                            ...
                            isCode:
                                lastMsg.isCode ||
                                (lastMsg.text.slice(-3) + token).includes('```') ||
                                token.includes('def ') ||
                    ...
                    setMessages((prev) => {
                        ...
                        if (lastMsg && lastMsg.isStreaming && lastMsg.role === 'system') {
                            ...
                            updated[prev.length - 1] = {
                                ...
                                isCode: lastMsg.isCode || (lastMsg.text.slice(-3) + data.chunk).includes('```'),
                            };
                            return updated;
                        }
                        return prev;
    ...
    const mdComponentsGeneral = useMemo(
        () => ({
            ...
            code: ({ node, ...props }: any) => (
                <code
                    className={`overlay-inline-code-surface rounded px-1 py-0.5 text-xs font-mono whitespace-pre-wrap ${isLightTheme ? 'text-[#c4623e]' : 'text-[#e8a882]'}`}
                    {...props}
                />
            ),
            blockquote: ({ node, ...props }: any) => (
                <blockquote
                    className={`border-l-2 pl-3 italic my-2 ${isLightTheme ? 'border-[#d97757]/30 text-slate-600' : 'border-[rgba(217,119,87,0.30)] overlay-text-secondary'}`}
                    {...props}
                />
            ),
            a: ({ node, ...props }: any) => (
    ...
    const mdComponentsShorten = useMemo(
        () => ({
            p: ({ node, ...props }: any) => <p className="mb-2 last:mb-0" {...props} />,
            strong: ({ node, ...props }: any) => (
                <strong className={`font-bold ${isLightTheme ? 'text-cyan-800' : 'text-cyan-100'}`} {...props} />
            ),
    ...
    const mdComponentsRecap = useMemo(
        () => ({
            p: ({ node, ...props }: any) => <p className="mb-2 last:mb-0" {...props} />,
            strong: ({ node, ...props }: any) => (
                <strong className={`font-bold ${isLightTheme ? 'text-[#b05530]' : 'text-[#faf9f5]'}`} {...props} />
            ),
            ul: ({ node, ...props }: any) => <ul className="list-disc ml-4 mb-2" {...props} />,
            li: ({ node, ...props }: any) => <li className="pl-1" {...props} />,
        }),
    ...
    const mdComponentsFollowUp = useMemo(
        () => ({
            p: ({ node, ...props }: any) => <p className="mb-2 last:mb-0" {...props} />,
            strong: ({ node, ...props }: any) => (
                <strong className={`font-bold ${isLightTheme ? 'text-amber-800' : 'text-[#FFF9C4]'}`} {...props} />
            ),
            ul: ({ node, ...props }: any) => <ul className="list-disc ml-4 mb-2" {...props} />,
            li: ({ node, ...props }: any) => <li className="pl-1" {...props} />,
        }),
    ...
    const mdComponentsWhatToAnswer = useMemo(
        () => ({
            p: ({ node, ...props }: any) => <p className="mb-2 last:mb-0" {...props} />,
            strong: ({ node, ...props }: any) => (
                <strong className={`font-bold ${isLightTheme ? 'text-emerald-700' : 'text-emerald-100'}`} {...props} />
            ),
            em: ({ node, ...props }: any) => (
                <em className={`italic ${isLightTheme ? 'text-emerald-700/80' : 'text-emerald-200/80'}`} {...props} />
            ),
            ul: ({ node, ...props }: any) => <ul className="list-disc ml-4 mb-2 space-y-1" {...props} />,
            ol: ({ node, ...props }: any) => <ol className="list-decimal ml-4 mb-2 space-y-1" {...props} />,
            li: ({ node, ...props }: any) => <li className="pl-1" {...props} />,
    ...
    const mdComponentsStandard = useMemo(
        () => ({
            p: ({ node, ...props }: any) => <p className="mb-2 last:mb-0 whitespace-pre-wrap" {...props} />,
            strong: ({ node, ...props }: any) => (
        ...
        if (msg.isCode || (msg.role === 'system' && msg.text.includes('```'))) {
            ...
            return (
                <div className={`rounded-lg p-3 my-1 border ${subtleSurfaceClass}`} style={appearance.subtleStyle}>
                    <div
                        className={`flex items-center gap-2 mb-2 font-semibold text-xs uppercase tracking-wide ${isLightTheme ? 'text-[#c4623e]' : 'text-[#e8a882]'}`}
                    >
                        <Code className="w-3.5 h-3.5" />
                        <span>Code Solution</span>
                    </div>
                    <div
                        className={`space-y-2 text-[13px] leading-relaxed ${isLightTheme ? 'text-slate-800' : 'overlay-text-primary'}`}
                    >
                        {parts.map((part, i) => {
                            if (part.startsWith('```')) {
                                ...
                                if (match) {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div
                                                className={`flex items-center justify-between px-3 py-1.5 border-b ${codeHeaderClass}`}
                                            ...
                                            {/* overflow-x-auto: code lines don't wrap → stable block height,
                                                no jitter as the spring animates or window resizes */}
                            ...
                            return (
                                <div key={i} className="markdown-content">
                                    <ReactMarkdown
                                        ...
                                        components={mdComponentsGeneral}
                                    >
                                        {part}
                                    </ReactMarkdown>
                                </div>
        ...
        if (msg.intent === 'shorten') {
            return (
                <div className={`rounded-lg p-3 my-1 border ${subtleSurfaceClass}`} style={appearance.subtleStyle}>
                    ...
                    <div
                        className={`text-[13px] leading-relaxed markdown-content ${isLightTheme ? 'text-slate-800' : 'overlay-text-primary'}`}
                    >
                        <ReactMarkdown
                            remarkPlugins={[remarkGfm, remarkMath]}
                            rehypePlugins={[rehypeKatex]}
                            components={mdComponentsShorten}
                        >
                            {msg.text}
                        </ReactMarkdown>
                    </div>
                ...
                <div className={`rounded-lg p-3 my-1 border ${subtleSurfaceClass}`} style={appearance.subtleStyle}>
                    ...
                    >
                        <ReactMarkdown
                            remarkPlugins={[remarkGfm, remarkMath]}
                            rehypePlugins={[rehypeKatex]}
                            components={mdComponentsFollowUp}
                        >
                            {msg.text}
                        </ReactMarkdown>
                    </div>
                            ...
                            return (
                                <div key={i} className="markdown-content">
                                    <ReactMarkdown
                                        remarkPlugins={[remarkGfm, remarkMath]}
                                        rehypePlugins={[rehypeKatex]}
                                        components={mdComponentsWhatToAnswer}
                                    >
                                        {part}
                                    </ReactMarkdown>
                                </div>
        ...
        return (
            <div className="markdown-content">
                <ReactMarkdown
                    remarkPlugins={[remarkGfm, remarkMath]}
                    rehypePlugins={[rehypeKatex]}
                    components={mdComponentsStandard}
                >
                    {msg.text}
                </ReactMarkdown>
            </div>
    ...
    useEffect(() => {
        ...
        const tick = (ts: number) => {
            ...
            if (direction !== 0) {
                ...
                velocity =
                    Math.abs(target - velocity) <= step ? target : velocity + Math.sign(target - velocity) * step;
            } else {
    ...
    // Kick-based inertial scroll engine for global shortcut dispatching.
    // Each shortcut fire "kicks" velocity on one axis; a RAF loop integrates with
    // friction. Handles both vertical (chat scroll) and horizontal (code block scroll).
    ...
    useEffect(() => {
        ...
        // Walk up from <pre> elements to find the horizontal scrolling wrapper.
        ...
        const kick = (axis: 'vert' | 'horiz', direction: -1 | 1) => {
            ...
            if (axis === 'vert') {
                ...
                if (!target) return; // no scrollable code block visible
            }
            const a = state[axis];
    ...
    return (
        <div
            data-theme="dark"
            ref={contentRef}
            className="terminal-scope flex flex-col items-center w-fit mx-auto h-fit min-h-0 bg-transparent p-0 rounded-[20px] font-sans overlay-text-primary"
        >
            <AnimatePresence>
                {isExpanded && (
                    <motion.div
                    ...
                    >
                        ...
                        >
                            {/* ── TOP BAR ───────────────────────────────────────────── */}
                            <div
                                className="flex items-center justify-between px-3 py-2.5 shrink-0 draggable-area border-b border-white/[0.07]"
                            >
                                ...
                                <div className="flex items-center gap-2 no-drag">
                                    ...
                                    >
                                        <img
                                            src={icon}
                                            alt="LiveLens"
                                            className="w-[18px] h-[18px] object-contain force-black-icon"
                                            draggable="false"
                                ...
                                <div className="flex items-center gap-0.5 no-drag">
                                    ...
                                    <div
                                        className={`flex items-center gap-1 px-2 py-1 border rounded-lg ${controlSurfaceClass}`}
                                        style={appearance.controlStyle}
                                    ...
                                    <button
                                        ...
                                        title={
                                            isInterviewerCapturing
                                                ? 'Interviewer capture ON — click to pause'
                                                : 'Interviewer capture OFF — click to resume'
                                        }
                                    >
                                    ...
                                    <button
                                        onClick={() => {
                                            const s = !isMousePassthrough;
                                            setIsMousePassthrough(s);
                                            window.electronAPI?.setOverlayMousePassthrough?.(s);
                                        }}
                            ...
                            {/* ── QUICK ACTIONS ROW ─────────────────────────────────── */}
                            <div
                                className="flex flex-nowrap items-center gap-1.5 px-3 py-2 overflow-x-auto no-drag scrollbar-hide shrink-0 border-b-0"
                            >
                                <button
                            ...
                            {(messages.length > 0 || isProcessing) && (
                                <>
                                    <div
                                        ...
                                        className="flex-1 overflow-y-auto p-4 space-y-4 max-h-[clamp(300px,35vh,450px)] no-drag"
                                                    ...
                                                    <div key={msg.id} className="flex justify-end animate-fade-in-up">
                                                        <div className="flex flex-col items-end gap-[5px] max-w-[84%]">
                                                            <div className="flex items-center gap-[5px] pr-[1px]">
                                                                <span className="text-[9px] font-bold uppercase tracking-[0.12em] text-accent-primary">
                                                                    You
                                                                </span>
                                                                <svg
                                                            ...
                                                            <div
                                                                className="px-3 py-2 text-[13px] leading-[1.55] bg-white/[0.05] border border-white/[0.09] text-[rgba(226,229,237,0.78)]"
                                                                style={{ borderRadius: '12px 4px 12px 12px' }}
                                                            >
                                                                {msg.hasScreenshot && (
                                                                    <div className="flex items-center gap-1 text-[10px] mb-1.5 pb-1.5 border-b border-white/[0.08] text-[rgba(226,229,237,0.40)]">
                                                    ...
                                                    <div key={msg.id} className="flex justify-end animate-fade-in-up">
                                                        <div className="flex flex-col items-end gap-[5px] max-w-[84%]">
                                                            <div className="flex items-center gap-[5px] pr-[1px]">
                                                                ...
                                                                {msg.isStreaming && (
                                                                    <span className="w-[5px] h-[5px] rounded-full animate-pulse bg-accent-primary" />
                                                                )}
                                                            </div>
                                                            <div
                                                                className="px-3 py-2 text-[13px] leading-[1.55] bg-white/[0.05] border border-white/[0.09] text-[rgba(226,229,237,0.78)]"
                                                ...
                                                <div key={msg.id} className="flex justify-start animate-fade-in-up">
                                                    <div className="flex flex-col items-start gap-[5px] max-w-[88%]">
                                                        <div className="flex items-center gap-[5px] pl-[1px]">
                                                            <span className="text-[rgba(106,155,204,0.70)] text-[10px] leading-none">
                                                                ●
                                                            </span>
                                                            <span className="text-[9px] font-bold uppercase tracking-[0.12em] text-[rgba(106,155,204,0.70)]">
                                                                LiveLens
                                                            </span>
                                                            {msg.isStreaming && (
                                                                <span className="w-[5px] h-[5px] rounded-full animate-pulse bg-[#6a9bcc]" />
                                                            )}
                                                        </div>
                                                        {hasOwnCard ? (
                                                            ...
                                                            <div
                                                                className="text-left text-[13px] leading-relaxed px-[14px] py-[10px] w-full bg-white/[0.05] border border-white/[0.09]"
                                                        ...
                                                        {!msg.isStreaming && (
                                                            <div className="flex items-center gap-1.5 flex-wrap">
                                                                ...
                                                                <button
                                                                    ...
                                                                    style={appearance.chipStyle}
                                                                >
                                                                    <ChevronsDown className="w-3 h-3 opacity-70" />{' '}
                                                                    Shorter
                                                                </button>
                                                                <button
                                        ...
                                        {isProcessing && !messages.some((m) => m.isStreaming) && (
                                            <div className="flex justify-start">
                                                <div className="flex flex-col items-start gap-[5px]">
                                                    <div className="flex items-center gap-[5px] pl-[1px]">
                                                        <span className="text-[rgba(106,155,204,0.45)] text-[10px] leading-none">
                                                            ●
                                                        </span>
                                                        <span className="text-[9px] font-bold uppercase tracking-[0.12em] text-[rgba(106,155,204,0.45)]">
                                                            LiveLens
                                                        </span>
                                                    </div>
                                                    <div
                                                        className="flex items-center gap-[4px] px-[14px] py-[10px] bg-white/[0.04] border border-white/[0.08]"
                                                        style={{ borderRadius: '4px 12px 12px 12px' }}
                                                    >
                                                        <div
                                                            className="w-[3px] h-[10px] rounded-full animate-[listening_1.1s_ease-in-out_infinite] bg-[rgba(106,155,204,0.45)]"
                            ...
                            <div className="px-3 py-2.5 no-drag shrink-0">
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            <span className="text-[11px] font-medium overlay-text-primary flex-1">
                                                ...
                                                {attachedContext.length > 1 ? 's' : ''}
                                            </span>
                                            {/* Mode dropdown */}
                                            <div ref={modeDropdownRef} className="relative">
                                                {modeDropdownOpen && (
                                                    <div
                                                        className={`absolute bottom-full right-0 mb-1.5 w-52 rounded-xl border overflow-hidden z-50 shadow-2xl overlay-shell-surface`}
                                                        ...
                                                        {ANALYSIS_MODES.map((m) => (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="text-[11px] font-medium overlay-text-primary">
                                                                        {m.label}
                                                                    </div>
                                                                    <div className="text-[9px] overlay-text-muted truncate">
                                                                        {m.description}
                                                                    </div>
                                                                </div>
                                                                {analysisMode === m.id && (
                                                                    <svg
                                                                        className="w-2.5 h-2.5 overlay-text-interactive shrink-0"
                                                                        fill="none"
                                                                        viewBox="0 0 24 24"
                                                                        strokeWidth={2.5}
                                                                        stroke="currentColor"
                                                ...
                                                >
                                                    <span>
                                                        {ANALYSIS_MODES.find((m) => m.id === analysisMode)?.icon ??
                                                            '💬'}
                                                    </span>
                                                    <span>
                                                    ...
                                                    <ChevronDown
                                                        className={`w-2.5 h-2.5 transition-transform opacity-50 ${modeDropdownOpen ? 'rotate-180' : ''}`}
                                                    />
                                                </button>
                                            </div>
                                            {/* Solve */}
                                    ...
                                    {!inputValue && (
                                        <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none text-[13px] overlay-text-muted">
                                            ...
                                            <div className="flex items-center gap-1 opacity-80">
                                                {(shortcuts.selectiveScreenshot || ['⌘', 'Shift', 'H']).map(
                                                    (key, i) => (
                                ...
                                {isInterviewerSpeaking && (
                                    <div className="flex items-center gap-[5px] mt-[5px] pl-[2px]">
                                        <span className="w-[5px] h-[5px] rounded-full animate-pulse shrink-0 bg-accent-primary" />
                                        <span className="text-[10px] text-[rgba(217,119,87,0.65)]">
                                            Interviewer speaking — edit if needed
                                        </span>
                                    </div>
                                )}
                            </div>
