...
export type { IpcOk, AudioDevice, MeetingSummary, MeetingDetail, StoredCredentials, LlmConfig, LicenseDetails, KeybindEntry, PermissionsResult, ThemeMode, ThemeState, CalendarEvent, ModeRecord, ModeReferenceFile, ModeNoteSection, IntelligenceContext, RagQueryResult, RagQueueStatus, ProfileStatus, TrialStatus, DonationStatus, SttProviderName, SttStatusData, WhisperModelSize };
...
export interface ElectronAPI {
  ...
  getCurrentLlmConfig: () => Promise<LlmConfig>
  ...
  getStoredCredentials: () => Promise<StoredCredentials>
  // Permissions
  checkPermissions:     () => Promise<PermissionsResult>
  requestMicPermission: () => Promise<boolean>

  // Free Trial
  startTrial:     () => Promise<TrialStatus>
  ...
  setSttProvider: (provider: SttProviderName) => Promise<{ success: boolean; error?: string }>
  getSttProvider: () => Promise<string>
  getWhisperModelSize: () => Promise<WhisperModelSize>
  setWhisperModelSize: (size: WhisperModelSize) => Promise<{ success: boolean; error?: string }>
  checkWhisperModel: (modelSize: WhisperModelSize) => Promise<{ downloaded: boolean }>
  downloadWhisperModel: (modelSize: WhisperModelSize) => Promise<{ success: boolean; error?: string }>
  ...
  // STT Status Events
  onSttStatusChanged: (callback: (data: SttStatusData) => void) => () => void
  ...
  getIntelligenceContext: () => Promise<IntelligenceContext>
  ...
  // Modes
  modesGetAll: () => Promise<ModeRecord[]>
  modesGetActive: () => Promise<ModeRecord | null>
  ...
  modesGetNoteSections: (modeId: string) => Promise<ModeNoteSection[]>
  ...
  getRecentMeetings: () => Promise<MeetingSummary[]>
  getMeetingDetails: (id: string) => Promise<MeetingDetail>
  ...
  // Theme API
  getThemeMode: () => Promise<ThemeState>
  setThemeMode: (mode: ThemeMode) => Promise<void>
  onThemeChanged: (callback: (data: ThemeState) => void) => () => void
  ...
  ragQueryMeeting: (meetingId: string, query: string) => Promise<RagQueryResult>
  ragQueryLive: (query: string) => Promise<RagQueryResult>
  ragQueryGlobal: (query: string) => Promise<RagQueryResult>
  ragCancelQuery: (options: { meetingId?: string; global?: boolean }) => Promise<{ success: boolean }>
  ragIsMeetingProcessed: (meetingId: string) => Promise<boolean>
  ragGetQueueStatus: () => Promise<RagQueueStatus>
  ...
  profileGetStatus: () => Promise<ProfileStatus>
  ...
  licenseGetDetails: () => Promise<LicenseDetails>
